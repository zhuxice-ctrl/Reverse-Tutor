...
- 修复节点子会话返回路径：从子窗口返回时回到原节点弹窗/详情上下文，二次进入不会重复自动发送开场问题。
- 新增 `@` 节点子窗口记忆引用：在对话输入 `@` 可选择已有节点子窗口，把对应节点上下文带入当前回复。
- 重做闲聊便签节点：闲聊中冒出的学习线索会显示成清晰便签，不再当成机械诊断面板。
- `归入大纲` 改为真实锚点流程：点击后进入所属会话的锚点页，预填大纲锚点草稿，便于确认后接入学习脉络。
- 修正图谱集合关系：全局图谱是所有会话图谱的全集，单独会话图谱显示各自子集。
- 整理会话线索支持勾选历史会话预览后写入图谱，学习节点、闲聊便签和关系会做去重合并与补充。
- 优化图谱动作显示：内部动作分布不再暴露 `emote` / `probe` / `clue` / `ask` 等英文枚举，改为中文标签。
- 修复后台生成提示气泡、节点子窗口返回、便签操作按钮等移动端细节问题。
...
- 修复真机长按会话后，置顶、删除、重命名、导出菜单仍可能跑到屏幕外的问题。
- 修复升级后 WebView 可能继续读取旧 app shell 缓存，导致补丁看起来没有生效的问题。
- 移除轻聊天里的 60 字/1 句硬限制，恢复更自然的 120 到 300 字左右回复；学习流输出上限同步提高。
- 补丁 APK 命名为 `Reverse-Tutor-v0.19.3.apk`。

## v0.19.2 - 2026-06-05

- 修复手机端主题卡片里较长的主题说明文字超出卡片边界的问题。
- 修复手机端长按会话后，置顶、删除、重命名、导出弹窗可能跑到屏幕外的问题。
- 修复手机端图谱语义片段卡牌无法上下滑动切换的问题。
- 补丁 APK 命名为 `Reverse-Tutor-v0.19.2.apk`。

## v0.19.1 - 2026-06-05

- 聊天窗口背景改为上浅下深的光晕渐变，并加入轻微白色斜线层次，避免横线条带来的割裂感。
- 图片资料统一归入锚点，文件和图片按资料类型分类展示；移除独立图片页，降低切换页面时的卡顿。
- 记忆面板精简为图谱：节点详情改成报告式阅读，支持拖拽展开/收起、相关聊天语义卡片和只读聊天记录回看。
- 会话管理弹窗调整为置顶、删除、重命名、导出；会话状态改为睡眠、在线、离线，并同步显示在会话列表和聊天顶栏。
- 侧边栏改为备忘录分段，点击横线新增记录，长按文本块可弹窗删除。
- 恢复连续消息的语义合并节奏，轻聊天输出容量回到约 200-300 字，并修复退出会话后生成状态丢失导致无法继续回复的问题。
...
- 板块 9C：新增知识图谱检索与策略注入，支持相关概念、历史错因、偏好、曾误解和拒绝复习挂起项进入系统提示。
